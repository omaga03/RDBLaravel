...
<div class="container py-4">
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form action="{{ route('frontend.rdbprojectgroup.store') }}" method="POST">
                @csrf
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('pgroup_id') is-invalid @enderror" id="pgroup_id" name="pgroup_id" required>
                        ...
                        @foreach($groups as $group)
                            <option value="{{ $group->pgroup_id }}" {{ old('pgroup_id') == $group->pgroup_id ? 'selected' : '' }}>{{ $group->pgroup_nameTH }}</option>
                        @endforeach
                    </select>
