...
<div class="container py-4">
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <h4 class="mb-0"><i class="bi bi-journal-plus"></i> เพิ่มข้อมูลผลงานตีพิมพ์</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('frontend.rdbpublished.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('pub_name') is-invalid @enderror" id="pub_name" name="pub_name" value="{{ old('pub_name') }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="pub_name_journal" name="pub_name_journal" value="{{ old('pub_name_journal') }}">
                </div>
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select" id="pubtype_id" name="pubtype_id">
                            ...
                            @foreach($pubtypes as $type)
                                <option value="{{ $type->pubtype_id }}" {{ old('pubtype_id') == $type->pubtype_id ? 'selected' : '' }}>{{ $type->pubtype_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="pub_date" class="form-label">วันที่ตีพิมพ์</label>
                        <input type="date" class="form-control" id="pub_date" name="pub_date" value="{{ old('pub_date') }}">
                    </div>
                </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('researcher_id') is-invalid @enderror" id="researcher_id" name="researcher_id" required>
                            ...
                            @foreach($researchers as $r)
                                <option value="{{ $r->researcher_id }}" {{ old('researcher_id') == $r->researcher_id ? 'selected' : '' }}>{{ $r->researcher_nameTH }} {{ $r->researcher_surnameTH }}</option>
                            @endforeach
                        </select>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="pro_id" name="pro_id">
                        ...
                        @foreach($projects as $pro)
                            <option value="{{ $pro->pro_id }}" {{ old('pro_id') == $pro->pro_id ? 'selected' : '' }}>{{ $pro->pro_nameTH }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="pub_abstract" name="pub_abstract" rows="4">{{ old('pub_abstract') }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="pub_keyword" class="form-label">คำสำคัญ (Keywords)</label>
                    <input type="text" class="form-control" id="pub_keyword" name="pub_keyword" value="{{ old('pub_keyword') }}">
                </div>
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="pub_budget" class="form-label">งบประมาณสนับสนุน (บาท)</label>
                        <input type="number" step="0.01" class="form-control" id="pub_budget" name="pub_budget" value="{{ old('pub_budget') }}">
                    </div>
                    <div class="col-md-6 mb-3">
