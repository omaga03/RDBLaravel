...
<div class="container py-4">
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <h4 class="mb-0"><i class="bi bi-pencil"></i> แก้ไขโครงการวิจัย</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('frontend.rdbproject.update', $item->pro_id) }}" method="POST" enctype="multipart/form-data">
                @csrf @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('pro_nameTH') is-invalid @enderror" id="pro_nameTH" name="pro_nameTH" value="{{ old('pro_nameTH', $item->pro_nameTH) }}" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="pro_nameEN" name="pro_nameEN" value="{{ old('pro_nameEN', $item->pro_nameEN) }}">
                </div>
                
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label for="pro_code" class="form-label">รหัสโครงการ</label>
                        <input type="text" class="form-control" id="pro_code" name="pro_code" value="{{ old('pro_code', $item->pro_code) }}">
                    </div>
                    <div class="col-md-4 mb-3">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control" id="pro_budget" name="pro_budget" value="{{ old('pro_budget', $item->pro_budget) }}">
                    </div>
                </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="pt_id" name="pt_id">
                            ...
                            @foreach($projectTypes as $type)
                                <option value="{{ $type->pt_id }}" {{ old('pt_id', $item->pt_id) == $type->pt_id ? 'selected' : '' }}>{{ $type->pt_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="ps_id" name="ps_id">
                            ...
                            @foreach($projectStatuses as $status)
                                <option value="{{ $status->ps_id }}" {{ old('ps_id', $item->ps_id) == $status->ps_id ? 'selected' : '' }}>{{ $status->ps_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="strategic_id" name="strategic_id">
                            ...
                            @foreach($strategics as $strat)
                                <option value="{{ $strat->strategic_id }}" {{ old('strategic_id', $item->strategic_id) == $strat->strategic_id ? 'selected' : '' }}>{{ $strat->strategic_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control" id="pro_date_start" name="pro_date_start" value="{{ old('pro_date_start', $item->pro_date_start) }}">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="pro_date_end" class="form-label">วันที่สิ้นสุด</label>
                        <input type="date" class="form-control" id="pro_date_end" name="pro_date_end" value="{{ old('pro_date_end', $item->pro_date_end) }}">
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-bordered" id="researcher_table">
                        ...
                        <tbody>
                            @foreach($item->researchers as $index => $res)
                            <tr id="researcher_row_{{ $index }}">
                                <td>
                                    <select class="form-select" name="researcher_id[]">
                                        <option value="">-- เลือกนักวิจัย --</option>
                                        @foreach($researchers as $r)
                                            <option value="{{ $r->researcher_id }}" {{ $res->researcher_id == $r->researcher_id ? 'selected' : '' }}>{{ $r->researcher_nameTH }} {{ $r->researcher_surnameTH }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <select class="form-select" name="position_id[]">
                                        ...
                                        @foreach($positions as $p)
                                            <option value="{{ $p->position_id }}" {{ $res->pivot->position_id == $p->position_id ? 'selected' : '' }}>{{ $p->position_nameTH }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td>
                                    <input type="number" class="form-control" name="ratio[]" placeholder="%" min="0" max="100" value="{{ $res->pivot->ratio }}">
                                </td>
                                <td>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="pro_abstract" name="pro_abstract" rows="5">{{ old('pro_abstract', $item->pro_abstract) }}</textarea>
                </div>
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        @if($item->pro_abstract_file)
                            <div class="mb-2">
                                <a href="{{ Storage::url($item->pro_abstract_file) }}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-file-earmark"></i> ดูไฟล์เดิม</a>
                            </div>
                        @endif
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        @if($item->pro_file)
                            <div class="mb-2">
                                <a href="{{ Storage::url($item->pro_file) }}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-file-earmark"></i> ดูไฟล์เดิม</a>
                            </div>
                        @endif
...
<script>
    let rowCount = {{ count($item->researchers) > 0 ? count($item->researchers) : 1 }};
    function addRow() {
