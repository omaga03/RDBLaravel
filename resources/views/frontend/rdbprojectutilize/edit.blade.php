...
<div class="container py-4">
    <div class="card shadow-sm">
        <div class="card-header bg-white"><h4 class="mb-0"><i class="bi bi-pencil"></i> แก้ไขข้อมูลการนำไปใช้ประโยชน์</h4></div>
        <div class="card-body">
            <form action="{{ route('frontend.rdbprojectutilize.update', $item->utz_id) }}" method="POST" enctype="multipart/form-data">
                @csrf @method('PUT')
                <div class="mb-3">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="date" class="form-control" id="utz_date" name="utz_date" value="{{ old('utz_date', $item->utz_date) }}">
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="chw_id" name="chw_id">
                            ...
                            @foreach($changwats as $changwat)
                                <option value="{{ $changwat->chw_id }}" {{ old('chw_id', $item->chw_id) == $changwat->chw_id ? 'selected' : '' }}>{{ $changwat->chw_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="utz_department_name" name="utz_department_name" value="{{ old('utz_department_name', $item->utz_department_name) }}">
                </div>
                <div class="row">
                    <div class="col-md-6 mb-3">
                        <label for="utz_budget" class="form-label">งบประมาณ</label>
                        <input type="number" step="0.01" class="form-control" id="utz_budget" name="utz_budget" value="{{ old('utz_budget', $item->utz_budget) }}">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="utz_files" class="form-label">ไฟล์แนบ</label>
                        @if($item->utz_files)
                            <div class="mb-2">
                                <a href="{{ Storage::url($item->utz_files) }}" target="_blank" class="btn btn-sm btn-outline-primary"><i class="bi bi-file-earmark"></i> ดูไฟล์เดิม</a>
                            </div>
                        @endif
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="number" class="form-control" id="utz_year_id" name="utz_year_id" value="{{ old('utz_year_id', $item->utz_year_id) }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="utz_year_bud" class="form-label">ปีงบประมาณ</label>
                        <input type="number" class="form-control" id="utz_year_bud" name="utz_year_bud" value="{{ old('utz_year_bud', $item->utz_year_bud) }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label for="utz_year_edu" class="form-label">ปีการศึกษา</label>
                        <input type="number" class="form-control" id="utz_year_edu" name="utz_year_edu" value="{{ old('utz_year_edu', $item->utz_year_edu) }}">
                    </div>
                </div>
                <div class="mb-3">
